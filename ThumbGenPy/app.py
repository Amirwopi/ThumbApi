...
def upload(episodeId):
    url = f"https://thumb-gen-4edy270in-amirwopis-projects.vercel.app/gen/{episodeId}_large"
...
def thumbnail(episodeId):
    url = f"https://thumb-gen-4edy270in-amirwopis-projects.vercel.app/gen/{episodeId}_thumb"
    link = Upload_To_Postman(url)
    x = link.split("/")
    link = "/".join(x[:-1])
    data = get_postman_urls(link)
